...
class VisualPerformanceManager {
    ...
    async init() {
        // Register effect profiles first to ensure they're available for updateCSSVariables
    ...
    calculateTier(capabilities) {
        ...
        // Memory weight: 20%
        score += (capabilities.memory >= this.thresholds.highTier.minMemory) ? 20 :
                 (capabilities.memory >= this.thresholds.mediumTier.minMemory) ? 12 : 4;
    ...
    updateCSSVariables() {
        ...
        // Check if effect registry is properly initialized
        if (!this.effectRegistry || this.effectRegistry.size === 0) {
          console.warn('[VisualPerformanceManager] Effect registry not initialized, skipping CSS variable updates');
          return;
        }
