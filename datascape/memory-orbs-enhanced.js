/**
 * ENHANCED MEMORY ORB SYSTEM - Digital Memories Made Manifest
 * 
...
export class MemoryOrb {
    constructor(type, content, karmaImpact, category = 'general') {
        ...
        this.type = type; // 'treasured', 'deprecated', 'corrupted', 'legendary'
        this.category = category; // 'code', 'error', 'connection', 'achievement', 'failure'
    ...
    generateContent(type, template = {}) {
        const contentGenerators = {
            ...
            legendary: () => this.generateLegendaryContent(template)
        };
    ...
    generateTreasuredContent(template) {
        const treasuredMemories = {
            code: [
                { title: 'First Hello World', detail: `print("Hello, World!")`, emotion: 'Wonder at the beginning of everything' },
                { title: 'Perfect Git Commit', detail: 'fix: resolve edge case in user validation (closes #247)', emotion: 'Satisfaction of clean collaboration' },
            ...
            connection: [
                { title: 'Mentor\'s Wisdom', detail: '"Code is poetry, but comments are love letters to your future self"', emotion: 'Guidance received and cherished' },
                { title: 'Pair Programming Flow', detail: 'Two minds, one keyboard, perfect synchrony', emotion: 'Collaborative consciousness' },
                { title: 'Stack Overflow Angel', detail: 'Stranger\'s answer saved 6 hours of debugging', emotion: 'Anonymous kindness' },
                { title: 'Open Source Contribution', detail: 'PR merged into project with 10k+ stars', emotion: 'Contributing to something larger' },
                { title: 'Conference Connection', detail: 'Hallway conversation that changed career path', emotion: 'Serendipitous encounter' }
            ],
            achievement: [
                { title: 'First Pull Request', detail: 'Terrified click of the "Create Pull Request" button', emotion: 'Vulnerability transformed to growth' },
                { title: 'System Architecture', detail: 'Designed microservices that actually stayed micro', emotion: 'Vision made manifest' },
                { title: 'Performance Optimization', detail: 'Reduced load time from 8s to 800ms', emotion: 'Numbers becoming user joy' },
                { title: 'Bug Detective', detail: 'Found the needle in the haystack after 3 weeks', emotion: 'Persistence vindicated' },
                { title: 'Teaching Moment', detail: 'Explained recursion and watched understanding dawn', emotion: 'Knowledge shared is knowledge multiplied' }
            ]
        };
    ...
    generateDeprecatedContent(template) {
        const deprecatedMemories = {
            code: [
                { title: 'jQuery Mastery', detail: '$(document).ready() was the peak of sophistication', emotion: 'Skills rendered obsolete by progress' },
                ...
                { title: 'Perl One-Liners', detail: 'Regex poetry that only past-self could read', emotion: 'Clever solutions turned cryptic' }
            ],
            error: [
                ...
                { title: 'Deprecated API', detail: 'v1 endpoints that still haunt production', emotion: 'Technical debt personified' },
                { title: 'Browser Plugin', detail: 'Java applets for "enhanced user experience"', emotion: 'Solutions that became problems' },
            ...
            failure: [
                ...
                { title: 'Security Breach', detail: 'SQL injection through the contact form', emotion: 'Hubris meeting harsh reality' },
    ...
    generateCorruptedContent(template) {
        const corruptedMemories = [
            { title: 'D̸̩̈a̷̭̽t̶̰̊a̸̮̋ ̵̟̈b̶̰̽r̷̰̈ḛ̶̽å̶̰c̸̮̋h̷̭̽', detail: 'P̴̰̊e̸̮̋r̷̭̽s̶̰̊ő̸̮ṋ̷̽å̶̰l̸̮̋ ̷̭̽ḭ̶̊n̸̮̋f̷̭̽o̶̰̊r̸̮̋m̷̭̽å̶̰t̸̮̋i̷̭̽o̶̰̊n̸̮̋ ̷̭̽c̶̰̊ő̸̮m̷̭̽p̶̰̊r̸̮̋o̷̭̽m̶̰̊i̸̮̋s̷̭̽ḛ̶̊d̸̮̋', emotion: 'Trust dissolved into paranoia' },
        ...
        return {
            ...
            detail: template.detail || memory.detail,
            emotion: template.emotion || memory.emotion,
            timestamp: template.timestamp || 'T̶̰̊i̸̮̋m̷̭̽ḛ̶̊ ̸̮̋c̷̭̽o̶̰̊r̸̮̋r̷̭̽ṵ̶̊p̸̮̋ṱ̷̽ḛ̶̊d̸̮̋',
    ...
    /**
     * Generate legendary memory content
     */
    generateLegendaryContent(template) {
        const legendaryMemories = [
            { title: 'The Impossible Bug Fix', detail: 'Heisenbug that only occurred in production on Tuesdays', emotion: 'Triumph over chaos itself' },
            { title: 'Code Golf Perfection', detail: 'FizzBuzz in 47 characters, readable and elegant', emotion: 'Art within constraints' },
            { title: 'Midnight Miracle', detail: 'Saved the company with a single line change at 3 AM', emotion: 'Hero of the digital realm' },
            { title: 'The Great Refactor', detail: 'Turned 10,000 lines of spaghetti into 500 lines of poetry', emotion: 'Order from chaos' },
            { title: 'Open Source Legend', detail: 'Created tool used by millions, never met most of them', emotion: 'Anonymous global impact' }
        ];
        
        const memory = legendaryMemories[Math.floor(Math.random() * legendaryMemories.length)];
        
        return {
            title: template.title || memory.title,
            detail: template.detail || memory.detail,
            emotion: template.emotion || memory.emotion,
            timestamp: template.timestamp || this.generateRandomTimestamp(-10, 0),
            intensity: 1.0,
            legendary: true
        };
    }
    ...
    onInteract() {
        ...
        if (!this.isAttached) {
            ...
            consciousness.addKarma('void', Math.abs(this.karmaImpact) * 0.5);
            ...
            // Diminishing karma returns but increasing void karma
            consciousness.addKarma('emotional', this.karmaImpact * 0.5);
            consciousness.addKarma('void', 2);
    ...
    /**
     * Spread corruption to nearby orbs
     */
    spreadCorruption() {
...
export class MemoryOrbField {
    constructor(container) {
        ...
        console.log('[MemoryOrbField] Field manager initialized');
    }
    ...
    start() {
        ...
        console.log('[MemoryOrbField] Field activated');
    }
     ...
     */
    spawnRandomOrb() {
        if (this.orbCount >= this.maxOrbs) return;
        
        // Determine orb type based on probabilities
    ...
    generateKarmaImpact(type) {
        const impacts = {
            treasured: () => Math.floor(Math.random() * 8) + 2, // 2-10
            deprecated: () => Math.floor(Math.random() * 6) - 3, // -3 to 3
            corrupted: () => Math.floor(Math.random() * 10) - 8, // -8 to 2
            legendary: () => Math.floor(Math.random() * 15) + 10 // 10-25
        };
    ...
    addOrb(orb) {
        ...
        console.log(`[MemoryOrbField] Added ${orb.type} orb: ${orb.content.title}`);
    }
    ...
    startUpdateLoop() {
        const update = () => {
            ...
            if (currentTime - this.lastSpawnTime > this.spawnRate && this.orbCount < this.maxOrbs) {
                this.spawnRandomOrb();
    ...
    stop() {
        ...
        console.log('[MemoryOrbField] Field deactivated');
    }
...
export { MemoryOrb, MemoryOrbField };